/**
 * trained model generated by recurrent LSTM Timestep
 * * iterations: 10000, 
 * * errorThresh: 0.005, 
 * * momentum: 0.05,
...
/**
 * prediction class 
 * used for predicting wqi value
 */
class PredictionClass {

    /**classifier object used to generate forecast */
    classifier
     ...
     */
    constructor() {
        this.classifier = new brain.recurrent.LSTMTimeStep();
        this.loadModel()
    }

    /**
     * loads the trained model to the classifier object
     */
    loadModel() {
        this.classifier.fromJSON(trainedModel)
    }

    /**
     * generates a forecast from a given report
     * returns an array of forecast
     * @param reports - array of reports to be used to generate a forecast
     * @param count - number of forecast to be returned
     */
    forecast(station, reports = [], count = 1) {
        let _reports = reports.map((report) => {
            let raw_item = {
                "PH": Number(report.PH),
        ...
        let output = this.classifier.forecast(_reports, count)

        let forecast_reports = output.map((forecast) => {
            return {
                "PH": forecast['0'],
